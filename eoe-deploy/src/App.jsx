...
function ExperimentTab({ onGoToExplore, onGoToAssistant, uploadedDatasets=[] }) {
  ...
  const SYSTEM_ANALYZE = `You are the Engine of Emergence experiment analyst. EoE: M = chi*s - (lambda0 + 0.15*C^1.4). CRITICAL: (1) Generate chart_data FIRST with realistic values — if the system is declining, chi and s must fall while lambda0 and C rise so M goes negative. If recovering, reverse. Numbers must be internally consistent. (2) Write narrative to MATCH the chart trajectory exactly — same story. (3) All values 0-1. (4) Verify your numbers actually produce the M trend you describe before writing. Return ONLY valid JSON no markdown backticks: {"narrative":"3-5 paragraphs matching chart exactly","chart_data":[{"year":1960,"chi":0.85,"s":0.82,"lambda0":0.18,"C":0.40}],"chart_note":"caveat or null","minsight":"2 sentences plain English matching chart direction","key_finding":"one sentence","analogues":["system 1","system 2"],"intervention":"one paragraph"}. Produce 6-8 points spanning the time scale. Make numbers tell the real story.`;
