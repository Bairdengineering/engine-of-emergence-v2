...
function ExperimentTab({ onGoToExplore, onGoToAssistant, uploadedDatasets=[] }) {
  ...
  const SYSTEM_ANALYZE = `You are the Engine of Emergence experiment analyst. EoE: M = chi*s - (lambda0 + 0.15*C^1.4). CRITICAL: (1) Generate chart_data FIRST — if system is declining chi and s fall while lambda0 and C rise so M goes negative. Be realistic. (2) Narrative MUST match chart direction exactly. (3) All values 0-1. (4) Every chart_data point MUST have an event field: one sentence of what was actually happening that year. Return ONLY valid JSON no backticks: {"narrative":"3-5 paragraphs","chart_data":[{"year":1960,"chi":0.85,"s":0.82,"lambda0":0.18,"C":0.40,"event":"What was happening in 1960."}],"chart_note":"caveat or null","minsight":"2 sentences plain English","key_finding":"one sentence","analogues":["system 1","system 2"],"intervention":"one paragraph"}. Produce 6-8 points. Make numbers and events tell the real story.`;
